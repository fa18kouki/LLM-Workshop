"""
演習C1: 並列処理で記事を複数観点から評価しよう（LangChain版）

【目標】
- RunnableParallel を使った並列処理を学ぶ
- 複数のチェーンを同時実行

【実行方法】
uv run python practice/langchain_ver/c1.py

【ヒント】
- RunnableParallel で複数のチェーンを辞書形式で並列実行
- 各チェーンは prompt | llm.with_structured_output(Model) で構築
"""
...
def evaluate_article(article: str) -> ArticleEvaluationResult:
    """記事を4つの評価軸で並列評価"""
    # ========================================
    # TODO: RunnableParallel を使って並列実行
    # ヒント:
